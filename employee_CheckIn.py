...
while continue_reading:
    ...
    if status == MIFAREReader.MI_OK:
        #stime = 9:00
        rfid_access.execute("INSERT INTO attendance(employeeID,clockIn) VALUES ((SELECT employee_uid FROM employee where card_uid = ?) , time('%H:%M','now','localtime'),(strftime('%H','now','localtime') - strftime('%H','09:00'))|| ':' || (strftime('%M','now','localtime') - strftime('%M','09:00')) )",[card_uid])
